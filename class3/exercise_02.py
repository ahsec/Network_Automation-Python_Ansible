...
def compare_in_out(initial_values, new_values):
    ...
    return (ifInOctets_diff, ifOutOctets_diff, ifInUcastPkts_diff,
            ifOutUcastPkts_diff)
...
def main():
    snmp_dev = exercise_01.SNMP_v3_DEVICE(DEV_TO_MONITOR, USER_NAME, AUTH_KEY,
                                          ENCRY_KEY, SNMP_PORT)
    snmp_dev.snmp_oids = (
    ...
    graph1 = create_graph_file("InOutOctect.svg", "Input/Output Octects",
                               "Input Octects", inOct, "Output Octects",
                               outOct, timeline)
    graph2 = create_graph_file("InOutUnicast.svg", "Input/Output Unicast",
                               "Input Unicast packets", inUnic,
                               "Output Unicast Packets", outUnic, timeline)
