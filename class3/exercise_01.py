...
If the running configuration has changed, then send an email notification to yourself
identifying the router that changed and the time that it changed.

Note, the running configuration of pynet-rtr2 is changing every 15 minutes (roughly at 
0, 15, 30, and 45 minutes after the hour).  This will allow you to test your script in the lab environment
...
def compare(old_values, new_values):
    ...
    # [('ccmHistoryRunningLastChanged', '356898449'), ('ccmHistoryRunningLastSaved', '356412236'),
    ...
    if((RLC_new > RLC_old) and (RLS_new == RLS_old) and (SLC_new == SLC_old)):
        ret = 'Reboot or running config has been modified at {} secs'.format(uptime)
    elif((RLC_new == RLC_old) and (RLS_new > RLS_old) and (SLC_new > SLC_old) and
         (RLS_new == SLC_new)):
        ret = 'write memory has been issued at {} secs'.format(uptime)
    elif((RLC_new == RLC_old) and (RLS_new > RLS_old) and (SLC_new == SLC_old)):
        ret = 'show runn has been issued causing write terminal to be issued at {} secs'.format(uptime)
    else:
...
class SNMP_v3_DEVICE(object):
    ...
    def read_all_oids(self):
        ...
        for desc, an_oid, is_count in self.snmp_oids:
            self.snmp_data = snmp_helper.snmp_get_oid_v3(self.dev_tuple, self.snmp_user, oid=an_oid)
...
def main():
    for dev in DEVS_TO_MONITOR:
        ...
        while True:
            # We'll sleep for SCAN_TIME secs before reading again and comparing values
